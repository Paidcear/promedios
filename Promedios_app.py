...
def generar_pdf(examen_final, tareas_total, actividades_total, complementos_total, complemento_extra, complemento_extra_examen, complemento_extra_tareas, promedio_final, actividades, id_estudiante):
    ...
    pdf.cell(200, 10, f"Portada de la materia (2 puntos): {complemento_extra:.2f} puntos", ln=True)
    pdf.cell(200, 10, f"Puntaje Extra de Examen (1 punto): {complemento_extra_examen:.2f} puntos", ln=True)
    pdf.cell(200, 10, f"Puntaje Extra de Tareas (1 punto): {complemento_extra_tareas:.2f} puntos", ln=True)
    pdf.ln(10)  # Espacio adicional
...
if st.sidebar.button("Generar Reporte General"):
    if id_estudiante:  # Verificar que el ID no esté vacío
        ...
        st.write(f"**Portada de la materia (2 puntos)**: {complemento_extra:.2f} puntos")
        st.write(f"**Puntaje Extra de Examen (1 punto)**: {complemento_extra_examen:.2f} puntos")
        st.write(f"**Puntaje Extra de Tareas (1 punto)**: {complemento_extra_tareas:.2f} puntos")
